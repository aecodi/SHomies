...
prompt
prompt Creating table MULTA
prompt ====================
prompt
create table sistema.MULTA
(
  nromulta   NUMBER not null,
  idfichador NUMBER not null,
  monto      NUMBER not null,
  fecha      DATE not null,
...
alter table sistema.MULTA
  add constraint PK_MULTA primary key (NROMULTA)
  using index 
  tablespace SYSTEM
  pctfree 10
  initrans 2
...
alter table sistema.MULTA
  add constraint PK_MULTA_TRABAJADOR foreign key (IDFICHADOR)
  references sistema.TRABAJADOR (ID);
...
prompt
prompt Creating package ADMINISTRA_MULTA
prompt =================================
prompt
CREATE OR REPLACE PACKAGE SISTEMA.administra_multa IS
   -------------------------------------------------------------------------------------------------
   PROCEDURE registrar(i_idfichador   IN NUMBER,
                       i_monto        IN NUMBER,
                       i_fechasistema IN DATE,
                       o_nromulta     OUT NUMBER,
                       o_codigo       OUT NUMBER,
                       o_mensaje      OUT VARCHAR2);
   -------------------------------------------------------------------------------------------------
   PROCEDURE getmultaporfichadorayfecha(i_idfichador   IN NUMBER,
                                        i_fechaproceso IN DATE,
                                        o_montomulta   OUT NUMBER,
                                        o_codigo       OUT NUMBER,
                                        o_mensaje      OUT VARCHAR2);
   -----------------------------------------------------------------------------------------------
END administra_multa;
/
...
prompt
prompt Creating package ADMINISTRA_PERSONA
prompt ===================================
prompt
CREATE OR REPLACE PACKAGE SISTEMA.administra_persona IS

  -------------------------------------------------------------------------------------------------
  PROCEDURE nuevo(i_nombrecompleto IN VARCHAR2,
                  i_tipodocumento  IN VARCHAR2,
                  i_nrodocumento   IN VARCHAR2,
                  o_id             OUT NUMBER,
                  o_codigo         OUT NUMBER,
                  o_mensaje        OUT VARCHAR2);

  -------------------------------------------------------------------------------------------------
  PROCEDURE updatename(i_nombrecompleto IN VARCHAR2,
                       i_id             IN NUMBER,
                       o_codigo         OUT NUMBER,
                       o_mensaje        OUT VARCHAR2);

  -------------------------------------------------------------------------------------------------
END administra_persona;
/

prompt
prompt Creating package ADMINISTRA_PERSONANATURAL
prompt ==========================================
prompt
CREATE OR REPLACE PACKAGE SISTEMA.administra_personanatural IS

  -------------------------------------------------------------------------------------------------
  PROCEDURE nuevo(i_id              IN NUMBER,
                  i_nombres         IN VARCHAR2,
                  i_apellidopaterno IN VARCHAR2,
                  i_apellidomaterno IN VARCHAR2,
                  i_fechanacimiento IN DATE,
                  i_sexo            IN VARCHAR2,
                  o_codigo          OUT NUMBER,
                  o_mensaje         OUT VARCHAR2);

  -------------------------------------------------------------------------------------------------
  PROCEDURE updatename(i_nombre  IN VARCHAR2,
                       i_id      IN NUMBER,
                       o_codigo  OUT NUMBER,
                       o_mensaje OUT VARCHAR2);

  -------------------------------------------------------------------------------------------------
END administra_personanatural;
/
...
CREATE OR REPLACE PACKAGE SISTEMA.administra_reportes IS
  ...
  -------------------------------------------------------------------------------------------------
  PROCEDURE pago_fichaje_entre_fechas(i_fechainicio IN DATE,
                                      i_fechafinal  IN DATE,
                                      o_reporte     OUT SYS_REFCURSOR,
                                      o_codigo      OUT NUMBER,
                                      o_mensaje     OUT NUMBER);
...
CREATE OR REPLACE PACKAGE SISTEMA.administra_trabajador IS
  ...
  -------------------------------------------------------------------------------------------------
  PROCEDURE nuevo(i_id           IN NUMBER,
                  i_fechaingreso IN VARCHAR2,
                  i_idcargo      IN VARCHAR2,
                  o_codigo       OUT NUMBER,
                  o_mensaje      OUT VARCHAR2);

  -------------------------------------------------------------------------------------------------
  PROCEDURE changeestado(i_id      IN NUMBER,
                         i_estado  IN NUMBER,
                         o_codigo  OUT NUMBER,
                         o_mensaje OUT VARCHAR2);
...
CREATE OR REPLACE PACKAGE BODY SISTEMA.administra_detalleorden IS
  ...
  BEGIN
    o_codigo := 0;
    UPDATE sistema.detalleorden SET estado = 2 WHERE idOrden = i_idorden;
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error al obtener datos de la orden [' || SQLERRM || ']';
...
prompt
prompt Creating package body ADMINISTRA_MULTA
prompt ======================================
prompt
CREATE OR REPLACE PACKAGE BODY SISTEMA.administra_multa IS
   -------------------------------------------------------------------------------------------------
   PROCEDURE registrar(i_idfichador   IN NUMBER,
                       i_monto        IN NUMBER,
                       i_fechasistema IN DATE,
   ...
   BEGIN
      o_codigo   := 0;
      o_nromulta := sistema.administra_correlativo.getnroorden('MULTA');
      IF o_nromulta IS NOT NULL
      THEN
         INSERT INTO sistema.multa
            (nromulta,
             idfichador,
             monto,
             fecha,
      ...
      ELSE
         o_codigo  := 999;
         o_mensaje := 'Error obteniendo correlativo de multa';
      END IF;
      sistema.administra_correlativo.actualizar('MULTA', o_codigo, o_mensaje);
   EXCEPTION
      WHEN OTHERS THEN
         o_codigo  := 999;
         o_mensaje := 'Error registrando multa [' || SQLERRM || ']';
   END;
   -----------------------------------------------------------------------------------------------
   PROCEDURE getmultaporfichadorayfecha(i_idfichador   IN NUMBER,
                                        i_fechaproceso IN DATE,
   ...
   BEGIN
      ...
      FROM   sistema.multa
      WHERE  idfichador = i_idfichador
             AND fecha = i_fechaproceso
             AND estado = 1;

      IF o_montomulta IS NULL
      THEN
         o_montomulta := 0;
      END IF;
   EXCEPTION
      WHEN OTHERS THEN
         o_codigo  := 999;
         o_mensaje := 'Error al obtener monto de multa [' || SQLERRM || ']';
   END;
...
prompt
prompt Creating package body ADMINISTRA_PERSONA
prompt ========================================
prompt
CREATE OR REPLACE PACKAGE BODY SISTEMA.administra_persona IS
  ...
  BEGIN
    o_codigo := 0;
    o_id     := sistema.administra_correlativo.getnroorden('PERSONA');
    IF o_id IS NOT NULL
    THEN
      INSERT INTO sistema.persona
        (id,
         nombrecompleto,
         idtipodocumento,
         numerodocumento)
    ...
    ELSE
      o_codigo  := 999;
      o_mensaje := 'Error obteniendo correlativo de persona';
    END IF;
    sistema.administra_correlativo.actualizar('PERSONA',
                                              o_codigo,
                                              o_mensaje);
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error registrando persona [' || SQLERRM || ']';
  END;
  ...
  BEGIN
    o_codigo := 0;
    UPDATE sistema.persona
       SET nombrecompleto = i_nombrecompleto
     WHERE id = i_id;
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error modificando nombre persona [' || SQLERRM || ']';
  END;
...
prompt
prompt Creating package body ADMINISTRA_PERSONANATURAL
prompt ===============================================
prompt
CREATE OR REPLACE PACKAGE BODY SISTEMA.administra_personanatural IS
  ...
  BEGIN
    o_codigo := 0;
    INSERT INTO sistema.personanatural
      (id,
       nombres,
       apellidopaterno,
       apellidomaterno,
  ...
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error registrando persona natural[' || SQLERRM || ']';
  END;
  ...
  BEGIN
    o_codigo := 0;
    UPDATE sistema.personanatural SET nombres = i_nombre WHERE id = i_id;
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error modificando nombre persona [' || SQLERRM || ']';
...
CREATE OR REPLACE PACKAGE BODY SISTEMA.administra_reportes IS
  ...
  BEGIN
    ...
    OPEN o_reporte FOR
      SELECT (SELECT per.nombres
             ...
             deto.precioventa,
             deto.comision
      FROM   sistema.detallefichaje detf,
             sistema.detalleorden   deto
  ...
  BEGIN
    ...
    OPEN o_reporte FOR
      SELECT (SELECT pe.nombrecompleto
             ...
             (SELECT SUM(fi.monto) AS montofichaje
              FROM   sistema.detallefichaje fi
              WHERE  trunc(fi.fechaproceso) BETWEEN trunc(i_fechainicio) AND
                     trunc(i_fechafinal)
                     AND fi.idfichador = d.idfichador
                     AND fi.estado NOT IN (3)) AS totalFichaje,
             (SELECT SUM(mu.monto) AS montoMulta
              FROM   sistema.multa mu
              WHERE  trunc(mu.fecha) BETWEEN trunc(i_fechainicio) AND
                     trunc(i_fechafinal)
                     AND mu.idfichador = d.idfichador
                     AND mu.estado = 1) AS totalMulta,
  ...
  BEGIN
    ...
    OPEN o_reporte FOR
      SELECT (SELECT pe.nombrecompleto
              FROM   sistema.persona pe
              WHERE  pe.id = d.idfichador) nombrefichadora,
             SUM(d.monto) AS montoFichaje,
             (SELECT SUM(mu.monto) AS montoMulta
              FROM   sistema.multa mu
              WHERE  trunc(mu.fecha) BETWEEN trunc(i_fechainicio) AND
                     trunc(i_fechafinal)
                     AND mu.idfichador = d.idfichador
                     AND mu.estado = 1) AS montoMulta
      FROM   sistema.detallefichaje d
      WHERE  trunc(d.fechaproceso) BETWEEN i_fechainicio AND i_fechafinal
             AND d.estado NOT IN (3)
      GROUP  BY d.idfichador
      ORDER  BY nombrefichadora;
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error inesperado [' || SQLERRM || ']';
  ...
  BEGIN
    ...
    OPEN o_reporte FOR
      ...
      FROM   sistema.detalleorden   o,
             sistema.detallefichaje d
      WHERE  trunc(o.fechaproceso) BETWEEN trunc(i_fechainicio) AND
             trunc(i_fechafinal)
             AND d.idorden = o.idorden
...
CREATE OR REPLACE PACKAGE BODY SISTEMA.administra_sistema IS
  ...
  BEGIN
    ...
    WHERE  sis.fecha = (SELECT MAX(sys.fecha)
                        FROM   sistema.auditoriasistema sys);
  EXCEPTION
    WHEN no_data_found THEN
      o_codigo  := 1;
  ...
  BEGIN
    ...
    WHERE  trunc(det.fechaproceso) = trunc(i_fechasistema)
           AND EXISTS (SELECT 1
            ...
            WHERE  ord.estado = estadoprocesado
                   AND ord.id = det.idorden);
    SELECT nvl(SUM(mu.monto), 0)
    INTO   montoMulta
    FROM   sistema.multa mu
    WHERE  fecha = i_fechasistema
           AND estado = 1;
    o_monto := o_monto - montoMulta;
  ...
  BEGIN
    ...
    END IF;
    sistema.administra_correlativo.actualizar('DETALLECIERRE',
                                              o_codigo,
                                              o_mensaje);
  EXCEPTION
    WHEN OTHERS THEN
...
CREATE OR REPLACE PACKAGE BODY SISTEMA.administra_trabajador IS
  ...
  BEGIN
    o_codigo := 0;
    INSERT INTO sistema.trabajador
      (id,
       fechaingreso,
       idcargo,
       estado)
  ...
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error registrando trabajador[' || SQLERRM || ']';
  END;
  ...
  BEGIN
    o_codigo := 0;
    UPDATE sistema.trabajador SET estado = i_estado WHERE id = i_id;
  EXCEPTION
    WHEN OTHERS THEN
      o_codigo  := 999;
      o_mensaje := 'Error cambiando estado trabajador[' || SQLERRM || ']';
  END;
  ...
  BEGIN
    ...
    OPEN o_trabajadores FOR
      SELECT tra.id,
             ...
             estado,
             idcargo,
             (SELECT car.descripcion
                FROM sistema.cargo car
               WHERE car.id = tra.idcargo) AS descripcioncargo
        FROM sistema.trabajador tra, sistema.personanatural per
       WHERE per.id = tra.id
...
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (34, 'VALERIA', 'DNI', '00000034');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (35, 'ALEXANDRA', 'DNI', '00000035');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (36, 'CIELO', 'DNI', '00000036');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (37, 'JESSICKA', 'DNI', '00000037');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (38, 'MARIEL', 'DNI', '00000038');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (39, 'ANGIE', 'DNI', '00000039');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (40, 'MIA', 'DNI', '00000040');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (41, 'YULIANA', 'DNI', '00000041');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
values (42, 'PERLA', 'DNI', '00000042');
insert into sistema.persona (ID, NOMBRECOMPLETO, IDTIPODOCUMENTO, NUMERODOCUMENTO)
